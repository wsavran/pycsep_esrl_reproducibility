"""
 Executes the following Python scripts to recreate figures from Savran et al. - pyCSEP: A Python Toolkit for Earthquake Forecast developers. Figure 1 and Figure 8 are not generated programatically, and are not included in the output from this script.
 
 This script can be executed in a stand-alone mode, but will require that the data files (10.5281/zenodo.5717419) are downloaded and extracted into the top-level directory of the reproducibility package. This script should be executed automatically when the Docker container containing the pyCSEP environment is created.
...
def verify_file_manifest():
    ...
    if light and full:
        ...
        raise FileNotFoundError('Missing files unable to run reproducibility package. Try re-downloading from Zenodo. Contact wsavran [at] usc.edu for assistance.')
    return output
